...
const EmbersMaterial = shaderMaterial(
    {
        uTime: 0,
        uColor: new THREE.Color('#ff4500'), // Base Orange-red
    },
    // Vertex Shader
    `
    uniform float uTime;
...
const MoltenMaterial = shaderMaterial(
  ...
  `,
    ...
    void main() {
      ...
      // Colors
      vec3 hotMagma = vec3(1.0, 0.8, 0.2); // Bright Orange/Yellow
      vec3 coolCrust = vec3(0.1, 0.05, 0.05); // Dark Rock
      vec3 coldMetal = vec3(0.2, 0.2, 0.25); // Grey Metal
      ...
      gl_FragColor = vec4(finalColor + vec3(glow * 0.5, glow * 0.2, 0.0), 1.0);
    }
  `
);
...
const HeatHazeMaterial = shaderMaterial(
  ...
  `,
    ...
    void main() {
      ...
      // Background Color: Dark Studio
      vec3 bgColor = vec3(0.05, 0.05, 0.05); // Dark grey/black
      
      // Warm Glow Gradient from bottom
      vec3 warmGlow = vec3(0.2, 0.05, 0.0); // Deep orange/red
...
extend({ EmbersMaterial, HeatHazeMaterial, MoltenMaterial });
